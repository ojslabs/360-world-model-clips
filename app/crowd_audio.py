...
MODEL_PATH = Path(os.environ.get("FOOTBALL_VAD_MODEL") or data_dir() / "audio-models/silero-vad-v6.1.onnx")
...
FALLBACK_PATH = data_dir() / "audio/fallback-crowd.wav"
...
def _speech_seconds(paths):
    try:
        ...
        raise RuntimeError("The local speech check is unavailable. Run python -m app.crowd_audio --setup-model or supply a reviewed crowd recording.") from None
