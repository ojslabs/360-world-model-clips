...
class PortableBuildTests(unittest.TestCase):
    def test_copied_build_has_no_parent_repo_or_font_dependency(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            root.mkdir()
            shutil.copy(source / "build_ui.py", root)
            shutil.copytree(source / "_report_kit", root / "_report_kit", ignore=shutil.ignore_patterns("__pycache__"))
            shutil.copytree(source / "ui", root / "ui", ignore=shutil.ignore_patterns("out"))
            environment = {**os.environ, "PYTHONPATH": ""}
            def build():
                subprocess.run([sys.executable, root / "build_ui.py"], cwd=temporary,
