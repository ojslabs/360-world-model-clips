...
from app import orbit_loop
from app import server
from app import source_atmosphere
...
class OutputTests(unittest.TestCase):
    def setUp(self):
        ...
        self.audio_preparer = patch.object(source_atmosphere, "prepare_slow_source_bed", return_value={
            "path": str(self.location / "source-audio-fixture.wav"),
            "provenance": {"method": source_atmosphere.METHOD, "source_video_id": self.video_id}})
        self.prepare = self.audio_preparer.start()
        self.addCleanup(self.audio_preparer.stop)
    ...
    def _assemble(self, source, orbit, freeze, output, tail_seconds, **options):
    ...
    def test_completed_orbit_assembles_locally_and_repeat_reuses_output(self):
        with patch.object(composite, "assemble", side_effect=self._assemble) as assemble:
            first = server.assemble_highlight(self.video_id, "first", 10)
            repeated = server.assemble_highlight(self.video_id, "first", 10)
    ...
    def test_new_timing_recut_preserves_historical_thirteen_and_nineteen_second_outputs(self):
        ...
        with patch.object(composite, "assemble", side_effect=self._assemble):
            recut = server.assemble_highlight(self.video_id, "first", 10)
        self.assertEqual(recut["media"]["duration"], 20)
        self.assertEqual((recut["lead_seconds"], recut["tail_seconds"]), (4, 10))
    ...
    def test_reference_anchoring_reaches_the_compositor_without_another_provider_call(self):
        ...
        def assemble(source, orbit, freeze, output, tail_seconds, reference_anchored, **options):
        ...
        with patch.object(orbit_loop, "anchor_reference", side_effect=anchor) as closure, \
            ...
            result = server.assemble_highlight(self.video_id, "first", 10)
            self.assertEqual(server.assemble_highlight(self.video_id, "first", 10), result)
        closure.assert_called_once()
        self.generate.assert_not_called()
        self.assertTrue(result["reference_anchored"])
        self.assertIn(f"-matched-{source_atmosphere.METHOD}.mp4", result["url"])
        saved = server.load_project(self.video_id)["generations"][0]
    ...
    def test_audio_version_is_separate_and_missing_file_can_be_reassembled(self):
        with patch.object(composite, "assemble", side_effect=self._assemble) as assemble:
            original = server.assemble_highlight(self.video_id, "first", 10)
            with patch.object(source_atmosphere, "METHOD", "next_audio"):
                revised = server.assemble_highlight(self.video_id, "first", 10)
                (self.location / "exports/first" / Path(revised["url"]).name).unlink()
                recovered = server.assemble_highlight(self.video_id, "first", 10)
        self.assertEqual(assemble.call_count, 3)
        self.assertEqual(revised["id"], recovered["id"])
        self.assertNotEqual(original["id"], revised["id"])
        self.assertEqual(revised["media"]["duration"], 20)
        self.assertTrue((self.location / "exports/first" / Path(original["url"]).name).is_file())

    def test_every_source_uses_own_slow_bed_and_preserves_prior_crowd_edit(self):
        previous = {"id": "old-crowd", "lead_seconds": 8, "tail_seconds": 4,
                    "audio_version": crowd_audio.AUDIO_VERSION,
                    "url": f"/media/{self.video_id}/exports/first/old-crowd.mp4"}
        previous_path = self.location / "exports/first/old-crowd.mp4"
        previous_path.write_bytes(b"prior crowd edit")
        previous_bytes = previous_path.read_bytes()
        self.project["generations"][0]["composites"] = [previous]
        media.write_json(server.manifest(self.video_id), self.project)
        target = self.location / "exports/first"
        ...
        with patch.object(source_atmosphere, "prepare_slow_source_bed", return_value=bed) as prepare, \
            ...
            revised = server.assemble_highlight(self.video_id, "first", 10)
            self.assertEqual(server.assemble_highlight(self.video_id, "first", 10), revised)
    ...
    def test_every_video_uses_source_sound_without_reading_overrides(self):
        with patch.object(media, "read_json", side_effect=AssertionError("No audio override lookup")):
            for video_id in (self.video_id, "8o40mSS05iE", "new-music-1"):
                self.assertEqual(server.audio_mode(video_id), "source_slow_motion")
        with patch.object(composite, "assemble", side_effect=self._assemble) as assembled:
            result = server.assemble_highlight(self.video_id, "first", 10)
        self.prepare.assert_called_once()
        self.assertIs(assembled.call_args.kwargs["auto_crowd"], False)
        self.assertEqual(result["audio_version"], source_atmosphere.METHOD)
        self.generate.assert_not_called()
    ...
    def test_failed_source_audio_preparation_never_substitutes_borrowed_crowd(self):
        ...
        with patch.object(source_atmosphere, "prepare_slow_source_bed", side_effect=ValueError("Audio decoder failed")), \
                patch.object(composite, "assemble") as assembled:
            with self.assertRaisesRegex(ValueError, "Audio decoder failed"):
                server.assemble_highlight(self.video_id, "first", 10)
    ...
    def test_direct_cut_revision_bypasses_legacy_cache_and_preserves_previous_edit(self):
        with patch.object(composite, "assemble", side_effect=self._assemble):
            old = server.assemble_highlight(self.video_id, "first", 10)
        ...
        def assemble(source, orbit, freeze, output, tail_seconds, reference_anchored, resume_next_frame, **options):
            self.assertTrue(reference_anchored)
    ...
    def test_combined_outputs_preserve_selected_order_and_reuse_identical_reel(self):
        with patch.object(composite, "assemble", side_effect=self._assemble):
            first = server.assemble_highlight(self.video_id, "first", 10)
            second = server.assemble_highlight(self.video_id, "second", 10)
    ...
    def test_combined_outputs_reject_invalid_unknown_duplicate_or_missing_selection(self):
        with patch.object(composite, "assemble", side_effect=self._assemble):
            output = server.assemble_highlight(self.video_id, "first", 10)
    ...
    def test_tail_and_source_context_rejected_before_any_generation(self):
        ...
        with patch.object(server, "generation_status", return_value={"ready": True, "model": fal_camera.MODEL}):
            for freeze, tail in ((2, 10), (901, 10), (905, 10)):
                project = server.load_project(self.video_id)
    ...
    def test_near_end_run_records_available_tail_and_does_not_require_ten_seconds(self):
        self.project["candidates"][0]["freeze_time"] = 898
        media.write_json(server.manifest(self.video_id), self.project)
        with patch.object(server, "generation_status", return_value={"ready": True, "model": fal_camera.MODEL}), \
             patch.object(server, "task", return_value={"job_id": "bounded-job"}):
            started = server.start_generation(self.video_id, "fal-h3-max")
        saved = server.read_run(self.video_id, started["run_id"])
        self.assertEqual(saved["tail_seconds"], 10)
        self.assertGreater(saved["edit_window"]["actual_tail_seconds"], 0)
        self.assertLess(saved["edit_window"]["actual_tail_seconds"], 2)
        public = server.public_project(self.video_id)
        self.assertEqual(public["candidates"][0]["edit_window"], saved["edit_window"])
        self.assertGreater(public["frame_selection"]["max_time"], 899)
        self.generate.assert_not_called()
    ...
    def test_rerun_copies_original_verified_frame_without_changing_current_selection(self):
        ...
        self.assertEqual(saved["tail_seconds"], 10)
        self.assertEqual(saved["lead_seconds"], 4)
