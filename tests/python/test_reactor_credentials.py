...
from app import reactor_api as api
from app import reactor_credentials as credentials
...
class RemixHandoffTests(unittest.TestCase):
    def test_key_crosses_private_pipe_only_and_worker_uses_immutable_snapshot(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            class Worker:
                ...
                def poll(self):
                    if self.returncode is None:
                        request_path = self.args[self.args.index("--worker") + 1]
                        self.returncode = remix._worker(request_path, credential_key=self.stdin.sent.decode())
            ...
            self.assertEqual(workers[0].args[1:4], ["-m", "app.reactor_remix", "--worker"])
            self.assertEqual(workers[0].kwargs["cwd"], PROJECT_ROOT)
